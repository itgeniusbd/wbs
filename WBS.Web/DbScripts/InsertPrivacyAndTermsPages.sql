-- Insert Privacy Policy and Terms & Conditions Pages
...
BEGIN
    PRINT 'Pages table does not exist. Please run migrations first.';
    RETURN;
END
GO

-- Insert Privacy Policy Page (English)
IF NOT EXISTS (SELECT 1 FROM Pages WHERE Slug = 'privacy-policy')
BEGIN
...
<p>আমরা সময়ে সময়ে এই গোপনীয়তা নীতি আপডেট করতে পারি। আমরা এই পৃষ্ঠায় নতুন নীতি পোস্ট করে যে কোনো পরিবর্তনের বিষয়ে আপনাকে অবহিত করব।</p>',
        'Privacy Policy - WBS',
        'Learn about how Working Bangladesh Society collects, uses, and protects your personal information.',
        1,
        1,
        GETDATE(),
        'System'
    );
    PRINT 'Privacy Policy page inserted successfully.';
END
ELSE
BEGIN
...
-- Insert Terms & Conditions Page (English)
IF NOT EXISTS (SELECT 1 FROM Pages WHERE Slug = 'terms-conditions')
BEGIN
    INSERT INTO Pages (Title, TitleBn, Slug, Content, ContentBn, MetaTitle, MetaDescription, IsActive, ShowInFooter, CreatedAt, CreatedBy)
    VALUES (
        'Terms & Conditions',
...
ঠিকানা: হাউস - ১৫ (৫এ), রোড-০৮, ব্লক -সি, বসিলা গার্ডেন সিটি, মোহাম্মদপুর, ঢাকা-১২০৭
        ...
        'Terms & Conditions - WBS',
        'Read the terms and conditions for using Working Bangladesh Society website and services.',
        1,
        1,
        GETDATE(),
        'System'
    );
    PRINT 'Terms & Conditions page inserted successfully.';
END
ELSE
BEGIN
    PRINT 'Terms & Conditions page already exists.';
END
GO

-- Verify the inserted pages
SELECT Id, Title, TitleBn, Slug, IsActive, ShowInFooter, CreatedAt
FROM Pages
